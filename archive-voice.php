...
<!-- <div class="column-tag">
	<?php
	$tag_counter = 0;
	$voice_tags = get_terms('voice_tag');
	...
	?>
			...
			<a href="<?php echo esc_url(get_term_link($voice_tag, 'voice_tag')); ?>"><?php echo $voice_tag->name; ?></a>
	<?php
			$tag_counter++;
		endforeach;
	endif;
	...
	<div class="inner">
		...
		<div class="front-voice-line voice-line">
			<?php foreach ($voice_list as $index => $voice) : ?>
				<?php
				$today = date("Y-m-d");
				$posted = date('Y-m-d', strtotime($voice->post_date));
				...
				<div class="front-voice-item">
					...
					<div class="front-voice-info">
						<div class="front-voice-photo">
							<img class="front-voice-photo-img fit" src="<?php echo get_the_post_thumbnail_url($voice->ID); ?>" />
						</div>
